...
sentiment_df <- read.csv("sentiment_output.csv", stringsAsFactors = FALSE)
sentiment_df$created_at.x <- ymd(sentiment_df$created_at.x)
...
# --------------------------------------------------------------------------
  ...
  output$map <- renderLeaflet({
    COUNTY <- ifelse(input$fileOption == "Pasco", "Pasco", "Hillsborough")
    content <- paste(sep ="<br/>",
                     "<b>County: </b>", COUNTY,
                     "<b>Residents: </b>", "584,067"
    )
...
# --------------------------------------------------------------------------
  output$myplot1 <- renderWordcloud2({
    p1 <- wordcloud2(d1(), size=1, minRotation = -pi/6, maxRotation = -pi/6)
    p1
  })
...
# --------------------------------------------------------------------------
  ...
  output$sentimentPlot <- renderPlot({
    ...
    options(repr.plot.height = 25)  # Adjust the height as needed
    plot
  })
...
# --------------------------------------------------------------------------
  ...
  output$sentimentPlotDescription <- renderText({
    ...
    paragraph <- paste("The sentiment plot visualizes the distribution of sentiment over time. It shows the variation in sentiment scores across", total_months,
                       "months of data. The sentiment scores range from -1 to 1, where -1 represents negative sentiment, 0 represents neutral sentiment, and 1 represents positive sentiment. The plot includes a line graph that connects the sentiment scores for each month, allowing us to observe trends and patterns in sentiment over time. Additionally, a smooth line has been added using the loess method to provide a smoothed representation of the sentiment distribution. 
